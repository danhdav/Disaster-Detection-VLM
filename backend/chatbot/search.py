...
def _debug_query(name: str, payload: dict[str, Any], **context: Any) -> None:
    if logger.isEnabledFor(logging.DEBUG):
        logger.debug("RAG query pattern=%s payload=%s context=%s", name, payload, context)
...
def semantic_query(query_text: str, n_results: int = 5) -> dict[str, Any]:
    ...
    _debug_query("semantic_query", payload)
    return payload
...
def disaster_type_query(
    ...
    _debug_query("disaster_type_query", payload)
    return payload
...
def radius_query(
    ...
    _debug_query("radius_query", payload, lat=lat, lon=lon, radius=radius)
    return payload
...
def group_files_query(
    ...
    _debug_query("group_files_query", payload, file_count=len(filenames))
    return payload
...
def keyword_document_query(
    ...
    _debug_query("keyword_document_query", payload, keyword=keyword)
    return payload
...
def ids_get_pattern(ids: list[str]) -> dict[str, Any]:
    ...
    payload = {"ids": ids}
    _debug_query("ids_get_pattern", payload, id_count=len(ids))
    return payload
...
def route_prompt_to_query(
    ...
    *,
    ...
    _debug_query(
        "route_prompt_to_query",
        routed["query_payload"],
        route=route,
        has_exact_match=bool(exact_match),
        has_coord_match=bool(coord_match),
        used_last_file=bool(last_queried_filename),
        last_found_count=len(memory_group),
    )
    return routed
